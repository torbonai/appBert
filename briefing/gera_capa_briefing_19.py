#!/usr/bin/env python3
# Gera capa briefing #19 · quarta 27/05/2026
...
# A capa OFICIAL Windows deve ser regerada com scripts/capa_briefing_19.ps1 (Segoe UI + UTF-8 BOM)

from PIL import Image, ImageDraw, ImageFont
import os

W, H = 1080, 1920
OUT = "/sessions/affectionate-nice-franklin/mnt/tradingview-mcp-jackson/bertani_app/assets/img/stories/briefing_19_capa_27mai.png"
...
center_text("QUARTA · 27 MAI", fData, 140, TEXT)
center_text("EDIÇÃO Nº 19", fEdicao, 215, MUTED)
...
# ========== MANCHETE PRINCIPAL ==========
fH1 = F(72, bold=True)
mancheteColor = ORANGE  # filtro moderado + atenção IPCA-15
center_text("INFLAÇÃO É FEBRE", fH1, 355, mancheteColor)
center_text("IPCA 9H É TERMÔMETRO", fH1, 445, mancheteColor)

# ========== SUBMANCHETE ==========
fH2 = F(25)
center_text("Falcão CETIP confirma bear steepen miolo (+14 bps F29)", fH2, 595, TEXT2)
center_text("WIN AGUARDAR · WDO no radar LONG estrutural", fH2, 635, TEXT2)
center_text("Brent -3,3% overnight pode furar narrativa hawkish", fH2, 675, TEXT2)
...
# WIN1! · AGUARDAR (GRAY_PILL)
cardWinColor = GRAY_PILL
...
badgeWinText = "AGUARDAR"
badgeWinColor = GRAY_PILL
...
fFase = F(21)
d.text((padX + 35, cardY + 100), "Reacumulação Fase B · sem sweep PDL", font=fFase, fill=MUTED)

fScore = Fmono(32, bold=True)
fDir   = F(22, bold=True)
winScoreColor = MUTED
d.text((padX + 35, cardY + 145), "3,0/10", font=fScore, fill=winScoreColor)
d.text((padX + 175, cardY + 151), "Sem viés · 2 setups pós-IPCA", font=fDir, fill=MUTED)

# WDO1! · AGUARDAR borderline (GOLD - destaca mais favorável)
...
d.text((padX + 35, cardY2 + 100), "Acumulação B · Spring potencial 5.050", font=fFase, fill=MUTED)
wdoScoreColor = GOLD
d.text((padX + 35, cardY2 + 145), "5,5/10", font=fScore, fill=wdoScoreColor)
d.text((padX + 175, cardY2 + 151), "LONG cond. · break 5.050", font=fDir, fill=TEXT2)

# ========== BANNERS INFERIORES ==========
bnPad = 80
bnW = W - 2*bnPad

# Banner 1 · IPCA-15 9H ★★★ (WARN red - critical)
bn1Y = 1220
bn1H = 150
bn1Color = WARN
rounded_rect(bnPad, bn1Y, bnW, bn1H, 20, fill=bn1Color, outline=bn1Color, width=0)
fBnT = F(32, bold=True)
center_text("IPCA-15 MAIO · 9H ★★★", fBnT, bn1Y + 25, WHITE)
fBnS = F(28, bold=True)
center_text("Janela_evitar 09:00-10:00 OBRIGATÓRIA", fBnS, bn1Y + 78, WHITE)

# Banner 2 · LIVE #4 20H YOUTUBE (ACCENT blue - call to action)
bn2Y = 1400
bn2H = 150
bn2Color = ACCENT
rounded_rect(bnPad, bn2Y, bnW, bn2H, 20, fill=bn2Color, outline=bn2Color, width=0)
center_text("HOJE 20H · LIVE #4 YOUTUBE", fBnT, bn2Y + 25, WHITE)
center_text("Trading é Matemática · 7 fórmulas", fBnS, bn2Y + 78, WHITE)

# ========== EVENTOS DO DIA ==========
fAvLbl = F(22, bold=True)
fAv    = F(22)
center_text("EVENTOS DO DIA", fAvLbl, 1600, WARN)
center_text("09:00 IPCA-15 maio BR ★★★", fAv, 1640, TEXT2)
center_text("10:00 PEC 6x1 + Pesquisas Jota/Indexa ★★", fAv, 1680, TEXT2)
...
center_text("27 MAI 08:30 BRT", fHora, 1870, MUTED)
